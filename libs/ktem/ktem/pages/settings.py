...
#         self._embeddings = []
#
#         self._render_app_tab = bool(self._default_settings.application.settings)
#         self._render_index_tab = bool(self._default_settings.index.settings or any(sig.settings for sig in self._default_settings.index.options.values()))
#         self._render_reasoning_tab = bool(len(self._default_settings.reasoning.settings) > 1 or any(sig.settings for sig in self._default_settings.reasoning.options.values()))
#
#         self.on_building_ui()
#
#     def on_building_ui(self):
...
#         self.reasoning_tab()
#
#         self.setting_save_btn = gr.Button("Save changes", variant="primary", scale=1, elem_classes=["right-button"])
#
#     def on_subscribe_public_events(self):
#         if self._app.f_user_management:
#             self._app.subscribe_event(
...
#             self.password_change_btn.click(
#                 self.change_password,
#                 inputs=[self._user_id, self.password_change, self.password_change_confirm],
#                 outputs=[self.password_change, self.password_change_confirm],
#                 show_progress="hidden",
#             )
#             self.signout.click(
#                 lambda: (None, "Current user: ___", "", ""),
#                 inputs=[],
#                 outputs=[self._user_id, self.current_name, self.password_change, self.password_change_confirm],
#                 show_progress="hidden",
#                 js=signout_js,
#             ).then(
#                 self.load_setting,
...
#         self.current_name = gr.Markdown("Current user: ___")
#         self.signout = gr.Button("Logout")
#         self.password_change = gr.Textbox(label="New password", interactive=True, type="password")
#         self.password_change_confirm = gr.Textbox(label="Confirm password", interactive=True, type="password")
#         self.password_change_btn = gr.Button("Change password", interactive=True)
#
#     def change_password(self, user_id, password, password_confirm):
#         from ktem.pages.resources.user import validate_password
...
#                 with gr.Group(visible=idx == 0, elem_id=pn) as self._reasoning_mode[pn]:
#                     reasoning = reasonings.get(pn, None)
#                     if reasoning is None:
#                         gr.Markdown("**Name**: Description")
#                     else:
...
#             if result:
#                 user = result[0]
#                 return f"Current user: {user.username}"
#             else:
#                 return "Current user: ___"
